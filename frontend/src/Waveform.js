...
class Waveform extends Component {  
  ...
  componentDidMount() {
    ...
    const volumeSlider = document.querySelector(`#volumeSlider${this.props.id}`)
  ...
  render() {
    ...
    return (
      <div style={{height : '50px'}} >
        <button style={{marginLeft : isMobile ? '4px' : '0px'}}id="wavebtn" onClick={this.handlePlay} >
          <span style={{fontSize :isMobile ? '2.5em': '3em'}}>{!this.state.playing ? '►' : '❚❚'}</span>
        </button>
        <div style={{width : isMobile ? `155px` : '400px', position : 'relative', left : '-5px'}}>
        <div class="waveform "id={`waveform${this.props.id}`} />
        </div>
       
        <div style={{ color : 'white',position : 'relative', top : isMobile ? '-50px ':'-45px', left :isMobile ?'196px':'450px'}}>►{this.props.plays}</div>
        <audio id={`track${this.props.id}`} src={this.props.url} />
        { this.state.hasStarted ? <div style={{ width : '30px',zIndex : '30', position : 'relative', left : isMobile ?'196px':'450px', top : '-88px'}}>{Math.floor(this.state.current / 60) + ':' + ('0'+Math.floor(this.state.current % 60)).slice(-2)}</div> : ''}
        <div id={`volumeSlider${this.props.id}`} class="volume"  style={{ color : 'white', visibility : !this.state.playing || isMobile ? 'hidden' : '',position : 'relative',Left : isMobile ? '-50px' : '0px'}}>
              <i
                class=" ui volume down icon" id="volumeIcon" style={{zIndex : '1000', position : 'relative',top : '-84px',left : isMobile ?'50px': '280px',color : 'white'
                  
                  }}
              />
              <input style={{backgroundColor : '#'+this.props.color,position : 'relative',left : isMobile ?'0px': '280px',}}
                id="volumeSlider"
                class="volume-slider"
                type="range"
                name="volume-slider"
